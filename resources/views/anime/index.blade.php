...
<div class="container mx-auto p-6">
    ...
    <div class="flex flex-col md:flex-row md:space-x-4 mb-6 justify-center">
        ...
        {{-- genre filter --}}
        <form action="{{ route('anime.filterByGenre') }}" method="GET" class="flex mb-4 md:mb-0">
        ...
        {{-- sorting form --}}
        <form action="{{ route('anime.index') }}" method="GET" class="flex">
        ...
        @foreach($anime as $a)
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 hover:shadow-xl transition flex flex-col justify-between">
                ...
                <form action="{{ route('wishlist.store') }}" method="POST" class="mt-4">
                    ...
                    <button type="submit" class="bg-emerald-700 text-white px-4 py-2 rounded-lg hover:bg-emerald-900 focus:outline-none focus:ring-2 focus:ring-emerald-500">Add to Wishlist</button>
                </form>
            </div>
        @endforeach
    </div>
